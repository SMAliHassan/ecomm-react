...
const HeaderErp = ({ open }) => {
  const [openProfileDropdown, setOpenProfileDropdown] = useState(false);
  const [activeSidebar, setActiveSidebar] = useState(window.innerWidth >= 992);
    ...
    <React.Fragment>
      ...
      <aside
        className={`side-nav vertical-menu nav-menu-light scrollable ${
          !activeSidebar ? 'nav-menu-collapse' : 'is-opened'
        }`}
      >
        <div className="nav-logo">
          <Link to="/erp" className="w-100 logo">
            <img
              className="img-fluid"
              src={!activeSidebar ? logoFold : logo}
              ...
              <div className="header-nav-item-select cursor-pointer">
                <i
                  className={`nav-icon feather ${
                    !activeSidebar ? 'icon-menu' : 'icon-arrow-left'
                  }`}
                ></i>
              </div>
            </div>
